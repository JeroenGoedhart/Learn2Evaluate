...
# Here we apply Learn2Evaluate to this data set. We will only consider binary classification.
...
load("DNAmethylation.Rdata")
Ytrain <- dat[[2]] # response variable == normalized age
Xtrain <- data.frame(dat[[1]]) # covariates == normalized DNA methylation levels
remove(dat)
...
# Code below applies Learn2Evaluate for the three learners: lasso, ridge, and randomforest.
# The learning curve is fitted by an inverse power law and MSE minimization is used to determine the optimal training size
Lasso <- Learn2Evaluate(X=data.frame(Xtrain), Y = Ytrain, nmin = 20, nev=10, nrep = 50,
                        learner ="lasso", method = "MSE", curve = "IPL", measure = "PMSE")

Ridge <- Learn2Evaluate(X=data.frame(Xtrain), Y = Ytrain, nmin = 20, nev=10, nrep = 50,
                        learner = "ridge", method = "MSE", curve = "IPL", measure = "PMSE")
RF <- Learn2Evaluate(X=data.frame(Xtrain), Y = Ytrain, nmin = 20, nev=10, nrep = 50,
                     learner = "rf",method = "MSE", curve = "IPL", measure = "PMSE")

### Plotting the Curves ###

#Code to plot the curve
PlotCurve(RF, Add = F) # for first plot set "Add" to FALSE, because it is the first plot
PlotCurve(Lasso, Add = T) # If you want to add the curve to existing plot, keep "Add" to TRUE. If you only want to see the curve for ridge, change to "Add = FALSE"
PlotCurve(Ridge, Add = T) # If you want to add the curve to existing plot, keep "Add" to TRUE. If you only want to see the curve for ridge, change to "Add = FALSE"
